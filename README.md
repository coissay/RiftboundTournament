...
- **Side deck par manche** : sur la page d'un match de tournoi, chaque joueur de la table note ses échanges avec la réserve **manche par manche** : en Bo3 / Bo5 à partir de la 2ᵉ manche (la manche 1 se joue avec le deck inscrit, le deck repartant de zéro à chaque nouvel adversaire), en Bo1 directement pour l'unique manche. Pour chaque manche : cartes **sorties** du deck principal et **entrées** depuis la réserve (quantités par carte de la version jouée, ou saisie libre si le deck a disparu) plus une note, pendant le tournoi ou après coup ; le formulaire de la manche N est prérempli d'après la manche N−1. Ces échanges restent **cachés à tout le monde sauf lui tant que le tournoi n'est pas clôturé** ; à la clôture ils deviennent visibles (page match + pictogramme 🔁 dans les pairings), avec pour chaque manche un bouton « Deck joué en manche N » qui affiche la composition jouée : cartes entrées dans le deck principal (badge IN) et sorties en réserve (badge OUT). Les noms de cartes sont survolables (image).
