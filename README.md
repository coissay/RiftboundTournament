...
- **Deckbuilder** (`/deckbuilder`) : construction visuelle d'un deck façon [Piltover Archive](https://piltoverarchive.com/deckbuilder) — galerie du catalogue filtrable (recherche tolérante aux accents/apostrophes, domaines, type, extension, coût, tri), clic gauche pour ajouter / clic droit pour retirer, compteur « − N + » sous chaque carte et sur chaque ligne du deck, boutons ★ (champion) et R (réserve), badge « 🎨 N versions » ouvrant le sélecteur d'illustration (art alternatif, showcase, réimpression ; la version choisie est exportée avec son code, ex. `3 Fiora, Peerless (SFD-110a)`, et affichée partout). Panneau latéral par sections (Légende / Champion / Deck principal / Champs de bataille / Runes / Réserve) avec compteurs et rappels des règles (1 légende, 1 champion, 40 cartes champion inclus, 3 champs de bataille, 12 runes, max 3 exemplaires, domaines de la légende) en vert/orange sans jamais bloquer. Le brouillon est conservé dans le navigateur (localStorage). Export texte au format decklist (copier, ou « Enregistrer comme deck » qui préremplit le formulaire), import d'une decklist collée, et « Ouvrir dans le deckbuilder » depuis un deck existant pour le modifier puis le mettre à jour.
